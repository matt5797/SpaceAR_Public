...
function update()
	...
	if moveCheck==false then
		...
		if CS.UnityEngine.Vector3.Distance(self.transform.position,originPos)>=1 then
		...
		dir = -self.transform.forward
		if CS.UnityEngine.Vector3.Distance(self.transform.position,originPos)<=0.1 then
