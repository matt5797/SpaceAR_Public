...
namespace SpaceAR.Editor.Views.Project
{
    [Binding]
    public class ProjectFolderViewModel : HierarchicalDataViewModel<ProjectItem>, IProjectFolderViewModel
    {
        ...
        protected override void OnContextMenu(List<MenuItemViewModel> menuItems)
        {
            if (TargetItem == null)
            {
                ...
                string LuaScriptStr = Localization.GetString("ID_RTEditor_ProjectFolderView_LuaScript", "LuaScript");
                ...
                CreateMenuItem(LuaScriptStr, LuaScriptStr.Replace(" ", ""), typeof(TextAsset), menuItems);
            }
            else
            {
                MenuItemViewModel createFolder = new MenuItemViewModel
        ...
        protected async virtual void CreateFolderContextMenuCmd(string arg)
        {
            ...
            string[] existingNames = parentFolder.Children != null ?
                parentFolder.Children.Where(c => c.IsFolder).Select(c => c.Name).ToArray() :
                new string[0];

            string name = m_project.Utils.GetUniqueName(Localization.GetString("ID_RTEditor_ProjectFolderView_Folder", "Folder"), existingNames);
